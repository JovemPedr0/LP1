...
Imobiliaria::~Imobiliaria(){
    std::cout << "Deletando Imobiliária" << std::endl;
     for (unsigned int i = 0; i < nEstoque.size(); i++){
        delete nEstoque[i];
    }
...
void Imobiliaria::adicionaImovel(Imovel *imovel){
    nEstoque.push_back(imovel);
}

void Imobiliaria::buscaImovel(std::string busca){
    for(unsigned int i = 0; i < nEstoque.size(); i++){
        if(nEstoque[i]->getNomeImovel() == busca || nEstoque[i]->getCondicaoImovel() == busca){
            nEstoque[i]->exibeAtributos();
        }
    }
}
...
void Terreno::exibeAtributos(){
    ...
    getAreaDoTerreno() << "m2 de área de terreno. R$ " << getValorImovel() << ". " << std::endl;
}
